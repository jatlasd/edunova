...
const MultipleSelector = React.forwardRef((props, ref) => {
  ...
  return (
    ...
    >
      <div
        className={cn(
          'min-h-10 rounded-md border border-primary/30 bg-white-1 text-sm ',
          {
      ...
      <div className="relative">
        {open && (
          <CommandList className="absolute top-1 z-10 w-full rounded-md border bg-white-1 text-popover-foreground shadow-md outline-none animate-in">
            {isLoading ? (
              <>{loadingIndicator}</>
            ) : (
              <>
